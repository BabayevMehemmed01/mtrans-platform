      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="px-2 py-1 rounded-lg hover:bg-accent text-xs text-muted-foreground transition-colors"
          >
            Bu gün
          </button>
          <button
          ...
          return (
            <button
              ...
              className={cn(
                ...
                dayTasks.length > 0 && "hover:bg-accent cursor-pointer"
              )}
            >
              <span className={cn(isToday(day) && "font-bold text-primary")}>{format(day, "d")}</span>
              {dayTasks.length > 0 && (
          ...
          <div className="space-y-2">
            ...
            {selectedTasks.map((task) => (
              <Link
                key={task.id}
                href={task.href}
                className="flex items-center gap-2 px-3 py-2 rounded-lg border border-border hover:border-primary/40 hover:bg-primary/5 transition-colors group"
              >
                <span
                  className={cn(
                ...
                />
                <span className="flex-1 min-w-0 text-sm text-foreground truncate group-hover:text-primary">
                  {task.title}
                </span>
                {task.meta && (
