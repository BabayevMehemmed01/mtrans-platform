...
import { useRouter } from "next/navigation"; // YENİ: Yönləndirmə üçün
...
export function UserProfileDropdown({ user }: UserProfileDropdownProps) {
  const router = useRouter(); // Səhifələrarası keçid üçün
  const [isViewOpen, setViewOpen] = useState(false);
  const [isEditOpen, setEditOpen] = useState(false);
  const [isCompanyOpen, setCompanyOpen] = useState(false);
  const [activeTab, setActiveTab] = useState("details");

  // DÜZƏLİŞ: Rolun adında "ADMIN" və ya "OWNER" keçirsə, Super Admin sayılır.
  const isSuperAdmin = user.role?.toUpperCase().includes("ADMIN") || user.role?.toUpperCase().includes("OWNER");
  ...
  const handleNavigation = (path: string) => {
    setViewOpen(false); // Modalı bağla
    router.push(path);  // Səhifəyə get
  };
    ...
    <>
      ...
      <CustomModal isOpen={isViewOpen} onClose={() => setViewOpen(false)} title="İstifadəçi Profili" maxWidth="max-w-4xl">
        ...
        <div className="animate-in fade-in">
              ...
              <div className="relative rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 h-32 overflow-visible shadow-sm">
                <div className="absolute -bottom-10 left-6">
                  <Avatar className="w-24 h-24 shadow-xl border-4 border-white bg-white">
                    <AvatarImage src={user.avatar} />
                    <AvatarFallback className="bg-slate-100 text-blue-700 font-black text-3xl">{user.name.substring(0, 2).toUpperCase()}</AvatarFallback>
                  </Avatar>
                </div>
              </div>
          ...
          {/* DİNAMİK TƏBLƏRİN YENİ DİZAYNI VƏ YÖNLƏNDİRMƏSİ */}
          {activeTab === "projects" && (
            <div className="text-center py-12 px-4 bg-slate-50 rounded-2xl border border-gray-200/50">
              <Folder className="w-16 h-16 text-blue-200 mx-auto mb-4" />
              <h3 className="text-xl font-black text-slate-800 mb-2">Layihələriniz</h3>
              <p className="text-sm font-medium text-slate-500 mb-6 max-w-sm mx-auto">Sizə aid olan aktiv və tamamlanmış layihələrin tam siyahısını görmək üçün Layihələr səhifəsinə keçid edin.</p>
              <button onClick={() => handleNavigation('/dashboard/projects')} className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-bold shadow-md transition-all active:scale-95">
                Layihələrə Keçid <ArrowRight className="w-4 h-4" />
              </button>
            </div>
          )}
          ...
          {activeTab === "tasks" && (
            <div className="text-center py-12 px-4 bg-slate-50 rounded-2xl border border-gray-200/50">
              <CheckSquare className="w-16 h-16 text-amber-200 mx-auto mb-4" />
              <h3 className="text-xl font-black text-slate-800 mb-2">Tapşırıqlar (My Work)</h3>
              <p className="text-sm font-medium text-slate-500 mb-6 max-w-sm mx-auto">Özünüzə təyin edilmiş bütün tapşırıqları idarə etmək üçün "Mənim İşlərim" bölməsinə keçin.</p>
              <button onClick={() => handleNavigation('/dashboard/my-work')} className="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-6 py-3 rounded-xl font-bold shadow-md transition-all active:scale-95">
          ...
          {activeTab === "team" && (
            <div className="text-center py-12 px-4 bg-slate-50 rounded-2xl border border-gray-200/50">
              ...
              <h3 className="text-xl font-black text-slate-800 mb-2">Komanda Üzvləri</h3>
              <p className="text-sm font-medium text-slate-500 mb-6 max-w-sm mx-auto">Şirkət daxilindəki digər əməkdaşları və şöbə üzvlərini görmək üçün Komanda səhifəsinə keçin.</p>
              <button onClick={() => handleNavigation('/dashboard/members')} className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl font-bold shadow-md transition-all active:scale-95">
                Komandaya Keçid <ArrowRight className="w-4 h-4" />
              </button>
            </div>
          )}

          {/* Hələlik hazırlanmayan səhifələr üçün xəbərdarlıq (Təqvim, Fəaliyyət) */}
          {(activeTab === "events" || activeTab === "activity") && (
            <div className="text-center py-16 px-4">
              <Loader2 className="w-10 h-10 text-blue-400 animate-spin mx-auto mb-4" />
              <h3 className="text-lg font-black text-slate-800 mb-2">Məlumatlar inteqrasiya olunur...</h3>
              <p className="text-sm font-medium text-slate-500 max-w-sm mx-auto">
                Bu bölmənin API bağlantıları (Backend) qurulur. Tezliklə aktiv olacaq.
              </p>
            </div>
          )}
        </div>
        ...
        <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setEditOpen(false); }}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div className="space-y-1.5">
              ...
              <input type="email" defaultValue={user.email} className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-white text-[14px] font-semibold focus:border-blue-500 outline-none transition-all shadow-sm" />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <input defaultValue="+994550000000" className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-white text-[14px] font-semibold focus:border-blue-500 outline-none transition-all shadow-sm" />
            </div>
          </div>
        ...
        <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setCompanyOpen(false); }}>
          ...
          <div className="flex items-start gap-4 p-5 bg-gradient-to-r from-slate-50 to-blue-50/50 border border-gray-200/80 rounded-2xl mb-2">
            ...
            <div>
              ...
              <p className="text-[13px] font-medium text-slate-500 leading-relaxed">
                {isSuperAdmin 
                  ? "Siz Super Admin olaraq rəsmi şirkət rekvizitlərini buradan redaktə edə bilərsiniz. Bütün panellər açıqdır." 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input defaultValue="www.m-trans.az" disabled={!isSuperAdmin} className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-white text-[14px] font-bold focus:border-blue-500 outline-none disabled:bg-gray-50 disabled:text-gray-500 transition-colors pl-10 shadow-sm" />
          ...
          <div className="pt-4 mt-2 border-t border-gray-100 flex justify-end gap-3">
            <button type="button" onClick={() => setCompanyOpen(false)} className="px-6 py-2.5 rounded-xl text-[13px] font-bold text-slate-600 hover:bg-slate-100 transition-colors">Bağla</button>
            {isSuperAdmin && (
              <button type="submit" className="px-8 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-[13px] font-bold shadow-md transition-all active:scale-95">Təsdiqlə və Yadda Saxla</button>
            )}
          </div>
        </form>
      </CustomModal>
